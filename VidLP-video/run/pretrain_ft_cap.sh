...
# exp_name=drama_ft_cap_lr5e5_from_Scratch_e30_seed0_from_TEST
exp_name=drama_pt_ft_cap
debug=False
...
load_path="result/drama_pt_seed0_from_/version_0/checkpoints/epoch_013-step_001794-val_score_1.45.ckpt" # model weights after pretraining + irtr fintuning
...
config_path=`python generate_configs.py with data_root=${DATA_ROOT} num_gpus=$_num_gpus num_nodes=$_num_nodes $config_name \
    per_gpu_batchsize=$per_gpu_batchsize val_check_interval=0.25 exp_name=$exp_name debug=$debug load_path=$load_path `
