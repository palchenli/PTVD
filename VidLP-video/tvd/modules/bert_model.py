...
class BertSelfOutputWithGate(nn.Module):
    ...
    def forward(self, hidden_states, input_tensor, gate):
        ...
        if self.gate_type == 'add1':
            hidden_states = gate * inter_hidden_states + (1-gate) * input_tensor
...
class BertCrossLayerWithGate(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
    ...
    def forward(
        ...
        layer_output_, layer_intermediate_output = apply_chunking_to_forward(
            self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, attention_output
        )
        gate = self.gating(layer_output_[:, :1])
        if detach_gate:
            gate_o = gate.detach()
        else:
            gate_o = gate
        layer_output = self.output_after_gate(gate_o * layer_intermediate_output, attention_output)
        outputs = (layer_output,) + outputs
