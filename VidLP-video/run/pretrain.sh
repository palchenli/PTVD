...
config_path=`python generate_configs.py with data_root=$DATA_ROOT num_gpus=$_num_gpus num_nodes=$_num_nodes $config_name \
    per_gpu_batchsize=$per_gpu_batchsize val_check_interval=0.25 exp_name=$exp_name debug=$debug batch_size=4096 max_steps=3000 warmup_steps=0.05 learning_rate=0.00005`
